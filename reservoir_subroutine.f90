SUBROUTINE reservoir_subroutine(variables that need to be passed)
   use Block_Routine
  ...
  ! ------------ calculate temperature change due to diffusion ---------------
      ...
      dif_epi_x  = v_t * area * density * heat_c * (temp_hypo(i-1) -
temp_epil(i-1))
      dif_hyp_x  = v_t * area * density * heat_c * (temp_epil(i-1) -
temp_hypo(i-1))
  ...
  ! ------------------------- calculate dV/dt ---------------------------
    ! ---------------- epilimnion -------------
         dV_dt_epi = (flow_in_epi_x - flow_out_epi_x - flow_epi_hyp_x) * density
* heat_c * (temp_epil(i-1))


    ! ---------------- hypolimnion ------------
         dV_dt_hyp = (flow_in_hyp_x + flow_epi_hyp_x - flow_out_hyp_x)  *
density * heat_c * (temp_hypo(i-1))
   ...
   ! ------------------- calculate change in temperature  ---------------------
       ...
       ! ---------------- epilimnion -----------
          ...
          !----- calculate change in layer volume  -------
            ...
            temp_change_ep(i) = advec_in_epix - advec_out_epix  + energy_x +
dif_epi_x - dV_dt_epi
...
!
  ...
  !---------- calculate combined (hypo. and epil.) temperature of outflow -----
    ...
    epix = temp_epil(i)*(flow_out_epi_x/outflow_x)  ! portion of temperature
from epilim. 
    hypox= temp_hypo(i)*(flow_out_hyp_x/outflow_x)  ! portion of temperature
from hypol.
    temp_out_tot(i) = epix + hypox

!------------- write energy budget terms each time step and data to save -----
          write(*,*) dif_epi_x, temp_hypo(i-1), temp_change_hyp(i),advec_in_hypx &
